...
function mostrarEstadoAprobacion() {
  const box = document.getElementById("estadoBox");
  if (!box) return;
  const estado = EMPRENDEDOR.estado || "pendiente";

  if (estado === "pendiente") {
    box.className = "panel-message";
    box.style.cssText = "display:block;background:#FFF4E0;color:#8A5A00;padding:14px 18px;border-radius:8px;font-size:.88rem;margin-bottom:18px";
    box.textContent = "Tu cuenta está en revisión. Un administrador de Lojanova debe aprobarla antes de que tus productos aparezcan en el sitio público. Mientras tanto, puedes completar tu perfil y cargar tus productos sin problema.";
  } else if (estado === "rechazado") {
    box.className = "error-msg panel-message";
    box.style.display = "block";
    box.textContent = "Tu cuenta fue revisada y no fue aprobada. Si crees que es un error, contacta a la Prefectura de Loja.";
  } else {
