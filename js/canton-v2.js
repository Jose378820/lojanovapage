(function(){
 ...
 const products=document.getElementById('ct-products'), producers=document.getElementById('ct-producers');
 const chips=document.getElementById('ct-chips'),retry=document.getElementById('ct-retry');
 let catalog=null,pending=false,category='',page=0,producerPage=0;
 const size=12;
 ...
 function producerCard(p){
   const title=p.emprendimiento||p.nombre;
   return `<a class="ct-card" href="marca.html?id=${encodeURIComponent(p.id)}">${picture(p.foto_url,'producer',title)}<div class="ct-card-body"><span class="ct-category">${esc(catalog.producerCategory(p.id))}</span><h3>${esc(title)}</h3>${p.emprendimiento?`<span class="ct-brand">${esc(p.nombre)}</span>`:''}<span class="ct-card-cta">Conocer su historia ↗</span></div></a>`;
 }
 ...
 function render(){
   ...
   producerPage=Math.min(producerPage,Math.max(0,Math.ceil(result.producers.length/size)-1));
   ...
   producers.innerHTML=result.producers.slice(producerPage*size,(producerPage+1)*size).map(producerCard).join('')||'<p class="ct-empty">Aún no hay productores publicados vinculados a este cantón.</p>';
   pager('products',page,result.products.length);pager('producers',producerPage,result.producers.length);
 ...
 async function load(){
   ...
   try{catalog=core.index(await core.load(db,controller.signal));render();}
     ...
     controller.abort();
     status.textContent='No pudimos completar la consulta. No se muestran cifras parciales.';
     products.innerHTML='<p class="ct-empty">La información no está disponible en este momento. Intenta nuevamente.</p>';
     producers.innerHTML='';retry.hidden=false;
 ...
 root.addEventListener('click',event=>{
   ...
   if(target==='products')page+=Number(button.dataset.step);else producerPage+=Number(button.dataset.step);
   render();const grid=target==='products'?products:producers;
